...
本リポジトリは、実システムから個人情報、実データ、認証情報、投資判断ロジックを除外した**サニタイズ済み技術公開版**です。
...
- **自動取引・ポートフォリオ管理**
  - **Scheduler**: 市場の取引時間帯（前場・後場等）に合わせた柔軟なジョブ実行。
  - **Strategy Engine**: 複数の時間軸・目的別の運用枠を搭載。
  - **Trade Modes**: デフォルトの仮想取引（PAPER）と本番取引（REAL）の切り替え。
- **堅牢なリスク管理 (Risk Management)**
  - **Kill Switch**: ポートフォリオのドローダウンが規定値を超過した場合に、新規エントリーを抑止し、既存ポジションの決済は許可するフェイルセーフ機構。
  - **銀行口座（生活防衛費）保護**: 銀行残高はAI管理の対象外とし、読み取り専用として安全に隔離。（導入予定）
- **データパイプライン & AI分析**
  - yfinance (PAPER) および kabu STATION API (REAL) からの市場データ取得と、Tick Data WebSocket配信。
  - AIエージェントによるマーケットコンテキストの分析と投資判断の支援。
- **フルスタックダッシュボード**
  - 資産推移、パフォーマンス分析、システム状態をリアルタイムに可視化するWebインターフェース。
...
実システムでは認証情報をソースコードに保存せず、実行時にSSM/KMSの管理境界から取得します。公開版には実行時の設定、口座接続、発注モジュールを含めないため、実口座用の環境変数や認証情報を設定する必要はありません。
...
*Disclaimer: 本リポジトリは技術公開を目的としたものであり、投資助言、金融サービスの提供、金融取引の実行を目的とするものではありません。本番運用において生じた損失等については一切の責任を負いません。*
